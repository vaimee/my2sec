...
async function map_aw_events(user_graph,msg_datetime,events_obj){
	...
	for (event in events_obj){
		...
		if(SOURCE.source=="http://www.vaimee.it/sources/aw-watcher-working"){
			var event_forced_bindings=construct_window_event_forced_bindings(user_graph,events_obj[event])
			found_source++
		}else{
			if(SOURCE.source=="http://www.vaimee.it/sources/aw-my2sec"){
				var event_forced_bindings=construct_my2sec_event_forced_bindings(user_graph,events_obj[event])
				found_source++				
			}
		}
