...
class SPARQLbench {
  ...
  sparql(template,bindings){
    ...
    if(hasMultBinds==true){
      ...
      for(var jj=0;jj<updatesTemplates.length;jj++){
        ...
        //console.log(`Update #${jj}: ${updatesTemplates[jj]}`)
  ...
  _handleMultipleBindings(template,bindings,singleBindings,real_limit){
    ...
    //BUGFIX, CREDO SI MANGI L'ULTIMO STATO
    //console.log(`STATE: ${state}, CHAR: ${char}`)
    if(state=="closed_bracket" && char=="}"){
      //console.log(`[ STATE: CLOSED BRACKET n${nDupedStrings}]`)
      //console.log("> string to analyze:\n"+string)
      memStrings[nDupedStrings]=string;
      dupeString[nDupedStrings]="";
    ...
    for(var line=0;line<Ndupes;line++){
      ...
      //--------------L'INTERA LINEA E' ORA DUPLICATA----------
  ...
  _onClosedBracket(j,valuesTemplate,string,singleBindings){
    ...
    //SE SIAMO QUI ALLORA SONO PRESENTI MULTIPLE BINDINGS NELLA RIGA
    if(detector==1){
        ...
        for(var key in temp){
          if(temp[key].includes("_:")){
            ...
            if(!isSingleBinding){
              ...
              if(!this.shadowBindings.join(" ").includes(temp[key])){
                //console.log("adding new shadow binding")
                this.shadowBindings.push(temp[key])
                //console.log(this.shadowBindings)
              }else{
                //console.log("skipping adding shadow binding")
              }


              //console.log("Multiple binding, replace: "+temp[key])
              temp[key]=temp[key].replace(temp[key],temp[key]+"_"+j)
      ...
      //console.log("NO MULT BINDING DETECTED");
      //SE SIAMO QUI ALLORA POTREBBERO ESSERCI SHADOW BINDINGS
      
      var shadow_detected=false;
      Object.keys(this.shadowBindings).forEach(k=>{
        //console.log(this.shadowBindings[k])
        if(string.includes(this.shadowBindings[k])){
          shadow_detected=true;
        }
      })
      if(shadow_detected){
        //console.log("FOUND SHADOW BINDING!");
        if(string.includes("BIND")){
          //ALLORA SIAMO NEL CASO DI BIND
          ...
          Object.keys(this.shadowBindings).forEach(k=>{
            //console.log(this.shadowBindings[k])
            if(benchString.includes(this.shadowBindings[k])){
              benchString=benchString.replace(this.shadowBindings[k],this.shadowBindings[k]+"_"+j);
            }
          })  
          //console.log(benchString);  
          return benchString;     
