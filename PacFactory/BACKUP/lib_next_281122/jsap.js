...
class Jsap {
  ...
  async update(key,bindings){
    //console.log("#############################################")
    //console.log(`### STARTING UPDATE PROCEDURE FOR: (${key}) ###`)
    ...
    if(update instanceof Array){
        ...
        console.log("### [X] PERFORMING SPLIT UPDATE! ###")
        var res=[];
        for(var i=0; i<update.length;i++){
          console.log(`--> Message #${i}, updating ${update[i].length} chars\n${update[i]}`)
          res[i]=await this.api.update(update[i],config);
    ...
    console.log("#############################################")
    console.log("Update length: "+update.length+" characters")

    //throw new Error("MAO")
    console.log("PERFORMING NORMAL UPDATE")
