...
class SPARQLbench {
  ...
  sparql(template,bindings){
    ...
    if(!this._isquery(template) && this._hasMultipleBindings(bindings)){
      ...
      //console.log(singleBindings,JSON.stringify(multipleBindings))
    ...
    if(hasMultBinds==true){
      ...
      this._handleMultipleBindings(template,multipleBindings,singleBindings,real_limit)
      throw new Error("MAO")
  ...
  _handleMultipleBindings(template,bindings,singleBindings,real_limit){
    ...
    for(var i=0; i<template.length; i++){
      ...
      switch (state) {
        //==== INIT STATE====
        case "init":
          if(string.includes("WHERE")||string.includes("where")){
            state="where"
            string="";//RESET STRING
          }
          break;
        ...
        case "where":
          if(char=="{"){
            state="where_open_bracket"
            string="";
          }
          break;


        case "where_open_bracket":
          if(char=="{"){
            state="where_double_open_bracket"
            string="";
          }
          if(char=="}"){
            state="where_closed_bracket"
            closedIndex=i;
            string="";
          }
          break;
        
        case "where_double_open_bracket":
          if(char=="}"){
            state="where_open_bracket"
            string="";
          }
    ...
    var rightcut=template.slice(0,closedIndex)
    var leftcut=template.slice(closedIndex)
    console.log(rightcut)
    console.log(" ")
    console.log(leftcut)

    var values=this.create_values()

    template=rightcut+values+leftcut;
  ...
  _handleMultipleBindings_OLD(template,bindings,singleBindings,real_limit){
