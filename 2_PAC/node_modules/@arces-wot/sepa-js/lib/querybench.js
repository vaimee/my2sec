...
class SPARQLbench {
  ...
  sparql(template,bindings){
    ...
    //var tmpissimo=prefixes+" "+template
    //console.log(tmpissimo.slice(1160,1170))
  ...
  _onClosedBracket(j,valuesTemplate,string,singleBindings){
    //console.log(valuesTemplate.body[j])
    //var Ndupes=valuesTemplate.body.length;
    var detector=0;
    ...
    if(detector==1){
      ...
      //var j=0;
        ...
        for(var key in temp){
          ...
          if(this._is_bnode(temp[key])){
          //console.log("bnode detected")
            temp[key]=temp[key].replace(temp[key],temp[key]+"_"+j)
          }else if(temp[key].startsWith("?")){//NON PUOI FARE QUESTA ANALISI DOPO AVER SOSTITUITO I BINDINGS, NON E' SICURO.
  ...
  _is_bnode(string){
    if(string.length>2){
      if(string.startsWith("_")&&string.charAt(1)==":"){
        return true;
      }else{
        return false;
      }
