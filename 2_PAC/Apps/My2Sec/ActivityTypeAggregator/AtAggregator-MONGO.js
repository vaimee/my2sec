...
const MongoDbMessagesConsumer= require("../../../core/Pattern/My2Sec/MongoDbMessagesConsumer")
const UsersProfessionsConsumer= require("../../../core/Pattern/My2Sec/UsersProfessionsConsumer")
const AwMapper= require("../AwMapper")
/*###########################################
...
class AtAggregator{
  constructor(jsap_file){
    ...
    //HARD
    this.awTrainingEventsConsumer; //prende da mongo
    this.awTrainingEventsRemover; //rimuove da mongo
    this.awMapper; //mappa messaggi

    //EASY
    this.awTrainingActivitiesProducer; //le manda nel grafo modificato delle attività
    this.awValidatedActivitiesConsumer; //le prende dal grafo modificato delle attività
    this.awValidatedActivitiesRemover; //le rimuove dal grafo modificato delle attività

    //NONE
    this.awActivitiesProducer; //produce attività, questo NON DEVE CAMBIARE PER NESSUN MOTIVO
  }

  async initializeModules(){
    var professionEnv=process.env.PROFESSION_TYPE; //! TIPO DI AGGREGATORE
    if(!professionEnv){professionEnv=""}


    this.awMapper= new AwMapper(jsap_file)
