...
def GetTestActivityEvents(json_events,
    ...
    try:
        ...
        test = test.assign(nodeid= test[user_graph]) #bugfix, senza non riesce ad aggiornare il csv
        for i in range(len(test[user_graph])): #altro bugfix, i nodeid devono essere diversi
           test["nodeid"][i]="uuid:"+str(i)

        print(test)
        test=test.fillna('NaN')#bugfix, senza non riesce a stampare il json

        test = test.astype("string")
        #print(test)
        test[datetimestamp] = test[datetimestamp].apply(lambda x: x.replace(" ", "T"))
        #test[activity_type] = test[activity_type].apply(lambda x: x.replace("<NA>", "ciao"))
