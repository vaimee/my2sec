...
try:
    print(os.environ)
    configurations["csv_name_ai"]=os.environ['KNOWLEDGE_CSV_PATH']
    print("- Env variable 'KNOWLEDGE_CSV_PATH' set with value: "+str(configurations["csv_name_ai"]))
except:
    print("- Env variable 'KNOWLEDGE_CSV_PATH' not set, using default: "+str(configurations["csv_name_ai"]))
finally:
    pass'''
...
def resample_not_shutdown(dataframe,
    ...
    try:
        ...
        #! Datetime64 does not support sum operations
        ...
        tmp = pd.DataFrame(data={"Timestamp": events, "Not_Shutdown": shutdown})
        tmp["Timestamp"] = pd.to_datetime(tmp["Timestamp"], utc=True)
        tmp = tmp.set_index("Timestamp").resample("5min", label="right").first()
        print("Oh shit")
...
def AddDuration(json_events,
    ...
    try:
        ...
        #! IT DOES NOT LIKE DATETIME64
        tmp = pd.concat([tmp2, resample_not_shutdown(tmp, not_shutdow, datetimestamp)])
        tmp = DropDuplicates_and_sort(tmp, datetimestamp)

        # Convert datetimestamp column to datetime64 format
        #tmp[datetimestamp] = pd.to_datetime(tmp[datetimestamp], errors='coerce')
        # Drop rows with missing datetimestamp values
        #tmp = tmp.dropna(subset=[datetimestamp])
...
def RemoveLowDuration(dataframe,
    ...
    '''
    #! On pandas versions following the 1.5.3 groupby does not work if there are timestamps in the array.
    #! For that reason, the next line will remove the timestamps column from the dataframe.
    #! The original dataframe is not altered
    '''
    newdataframe=dataframe.drop(datetimestamp,axis=1)
    tmp = newdataframe.groupby(by=events_columns).sum().reset_index()
    #tmp = dataframe.groupby(dataframe.columns.difference(['Timestamp'])).sum().reset_index()
    #print(tmp)
...
def CreateCSV(json_file):
    ...
    if CheckCSV(configurations['csv_name_ai']):
        print("** Csv exists, concatenating")
...
def GetTestActivityEvents(json_events,
    ...
    try:
        ...
        #print("NAI")
        #print(train.user_graph)
        user = train.user_graph.iloc[0]
        #print("MAO")
        # load knowledge
        #!do not use knwoledge param to check csv, it uses default
        if CheckCSV(configurations['csv_name_ai']):
            print('Loading knowledge...')
            csv = pd.read_csv(configurations['csv_name_ai'])
            csv = csv[(csv.user_graph==user)&(csv.event_type==windowEvent_name)]
            train = pd.concat([train, csv])
        else:
            print("Knowledge csv does not exist, first run")
...
print("============================")
print("ACTIVITY TYPE AI FILTER LOGS:")
print("============================")
filename=sys.argv[1]
mode=sys.argv[2] #get_training_events / get_test_activity_events
knowledge_csv_path=sys.argv[3]
print(knowledge_csv_path)
configurations["csv_name_ai"]=knowledge_csv_path #OVERRIDE VARIABLE
print("KNOWLEDGE_CSV_PATH OVERRIDE WITH: "+str(configurations["csv_name_ai"]))
...
try:
    if mode=="get_training_events":
        ...
        print("Saving knowledge to CSV")
        CreateCSV(dataToSend)
        #dataToSend=dataToSend.replace("<NA>","NaN")
