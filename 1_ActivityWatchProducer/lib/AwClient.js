...
class AwClient {
	...
	constructor(jsap){
		...
		//console.log(jsap)
		this.aw_host=jsap.extended.AwProducer.endpoints.AwApiRouter
	}
	...
	load_configuration(jsonFilePath){ 
		return new Promise(resolve=>{
			...
			.then(data => {
				...
				log.info("awclient: parsed json (AW configuration file)")
				log.debug(JSON.stringify(this.jsonObj))
	...
	create_producer_bucket(){
		return new Promise(resolve=>{
			...
			var reqtext=this.aw_host+"/api/0/buckets/aw-producer";
	...
	create_event(json_event){
		return new Promise(resolve=>{
			...
			var reqtext= this.aw_host+"/api/0/buckets/aw-producer/events";
			req.open("POST", reqtext, true);
	...
	delete_event(event_ID){
		...
		return new Promise(resolve=>{
			...
			var reqtext=this.aw_host+"/api/0/buckets/aw-producer/events/"+event_ID;
...
class AwMy2SecClient extends AwClient{
	constructor(jsap){
		...
		this.my2secapi_host=jsap.extended.AwProducer.endpoints.My2secApi;
